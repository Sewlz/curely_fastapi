...
async def generate(input_data: InputData, service: LLMService = Depends(lambda: model_service)):
    try:
        response = service.generate_response(input_data.prompt, input_data.session_id, input_data.user_id)
...
async def get_session_messages(session_id: str, user_id: str = Form(...)):
    try:
